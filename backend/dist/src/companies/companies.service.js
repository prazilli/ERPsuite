...
let CompaniesService = class CompaniesService {
    ...
    async getProfile(companyId, currentUser) {
    ...
    async updateProfile(companyId, data, currentUser) {
        await this.verifyTenant(companyId, currentUser);
        const company = await this.prisma.company.findUnique({
            where: { company_id: companyId },
        });
    ...
    async getAnalytics(companyId, currentUser) {
        await this.verifyTenant(companyId, currentUser);
        const now = new Date();
        const thirtyDaysFromNow = new Date();
        thirtyDaysFromNow.setDate(now.getDate() + 30);
        const [totalEmployeesCount, activeEmployeesCount, inactiveEmployeesCount, attendanceStats, employeesOnLeaveCount, totalClientsCount, totalLeadsCount, totalLeadsWonCount, totalProjectsCount, activeProjectsCount, completedProjectsCount, delayedProjectsCount, revenuePaidAgg, expensePaidAgg, payrollPaidAgg, vendorBillsPendingAgg, invoicesUnpaidAgg, assetsStats, departmentsList, userCreatedList, leadList, projectList, invoiceList, vendorBillList, assetList, pendingApprovalsCount, unassignedUsers, unmanagedProfiles,] = await Promise.all([
