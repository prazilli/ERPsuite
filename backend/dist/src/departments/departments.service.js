...
let DepartmentsService = class DepartmentsService {
    ...
    async create(companyId, data, currentUser) {
    ...
    async update(id, data, currentUser) {
        ...
        if (!isCompanyAdmin && !isSuperAdmin) {
            throw new common_1.ForbiddenException('Only Company Head / CEO or Super Admin can modify department configurations.');
        }
        const updateData = {};
        if (data.name !== undefined)
            updateData.department_name = data.name;
    ...
    async getMetrics(id, currentUser) {
        const dept = await this.findOne(id, currentUser);
    ...
    async assignHead(id, targetUserId, currentUser) {
        const dept = await this.prisma.department.findUnique({
            where: { department_id: id },
        });
        if (!dept) {
        ...
        if (targetUser.company_id !== dept.company_id) {
            throw new common_1.ForbiddenException('Target employee belongs to another company.');
        }
        return this.prisma.$transaction(async (tx) => {
            ...
            if (!employeeRole || !deptHeadRole) {
                throw new common_1.BadRequestException('Required roles do not exist in database.');
            }
    ...
    async verifyCompanyAdmin(companyId, currentUser) {
        await this.verifyTenant(companyId, currentUser);
        if (currentUser.roleName !== 'Company Head / CEO' && currentUser.roleName !== 'Company Admin' && currentUser.roleName !== 'Super Admin') {
            throw new common_1.ForbiddenException('Only Company Administrators can perform this action.');
        }
    }
    verifyDepartmentIsolation(departmentId, currentUser) {
        if (currentUser.roleName === 'Super Admin' || currentUser.roleName === 'Company Head / CEO' || currentUser.roleName === 'Company Admin')
            return;
        if (currentUser.departmentId && BigInt(currentUser.departmentId) !== departmentId) {
            throw new common_1.ForbiddenException('Department isolation check failed: You are restricted to your assigned department.');
        }
    }
};
