...
export class MailService implements OnModuleInit {
  ...
  constructor() {
    ...
    try {
      this.transporter = nodemailer.createTransport({
        host,
        port,
        secure: false, // false for port 587 or other non-465 ports; will upgrade via STARTTLS if supported
        auth: {
          user,
          pass: pass || '',
        },
  ...
  async sendOtpEmail(recipientEmail: string, otpCode: string, firstName: string): Promise<void> {
    ...
    // Strict requested text body format
...
Amdox ERP`;
    ...
    // Beautiful styling matching user requirements and design guidelines
    const htmlBody = `
